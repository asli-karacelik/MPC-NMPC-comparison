function dxdt = function_state_continuous_opt_gaslift(x, u) % Continuous-time state equations for the gas lift model
% Input:
...
% Liquid flow rate (g/s)
    % Liquid inflow rate 
    wlin = (1 - alphagbot) * A * sqrt(rhol * max(0, pres - pbot));
    % Liquid outflow rate 
...
%% State equations
dxdt = zeros(3, 1);
dxdt(1) = wginj - wgout;      % Accumulation of gas (g/s)
dxdt(2) = wlin - wlout;       % Accumulation of liquid (g/s)
dxdt(3) = u(2);               % Unmeasured disturbance
