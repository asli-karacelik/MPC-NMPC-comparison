...
%% Get parameters
par = function_parameters_MPC_refchange_gaslift;

% % Store parameters in an Excel file
% T = {'Run', 'Sample Time (s)', 'Reference (liquid outflow rate [g/s])', 'Comment'};
% writecell(T, 'parameters_MPC_refchange_gaslift.xlsx', 'Sheet', 1, 'Range', 'A1:D1');
% T = table({'1'}, par.sample_time, par.wloutref);
% writetable(T, 'parameters_MPC_refchange_gaslift.xlsx', 'Sheet', 1, 'WriteVariableNames', 0, 'Range', 'A2:C2');
% winopen('parameters_MPC_refchange_gaslift.xlsx')

%% Linearization
% The internal plant model is obtained by linearizing a nonlinear plant model,
% |model_refchange_gaslift|, in Simulink at a nonzero steady-state operating point.

% Generate an operating point specification for the model's initial conditions.
plant_model = 'model_refchange_gaslift';
sim(plant_model)
...
savefig(fig, 'fig_MPC_ref30_Ts0_1_ph10_ch2_gaslift.fig') 
close(fig)
