...
%% Get parameters.
par = function_parameters_MPC_PMM_mg_gaslift;

% % Store parameters in an Excel file
% T = {'Run', 'Sample Time (s)', 'Prediction Horizon', 'Control Horizon', 'Covariance Matrix (Q)', 'Comment'};
% writecell(T, 'parameters_MPC_PMM_mg_gaslift.xlsx', 'Sheet', 1, 'Range', 'A1:F1');
% T = table({'1'}, par.sample_time, par.prediction_horizon, par.control_horizon);
% writetable(T, 'parameters_MPC_PMM_mg_gaslift.xlsx', 'Sheet', 1, 'WriteVariableNames', 0, 'Range', 'A2:D2');
...
plant_model = 'model_PMM_mg_gaslift';
sim(plant_model)
...
Q = [1.203098744993300e-12,-8.845009938166915e-09,0;-8.845009938166915e-09,6.502724828850785e-05,0;0,0,4]; %B_est * B_est';
R = D_est * D_est';
N = B_est * D_est';
...
savefig(fig, 'fig19_MPC_PMM_mg_Ts0_1_ph10_ch2_QTs2_gaslift.fig') 
close(fig)
